...
function thumb(res, id) {
  ...
  res.end(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 180">' +
      '<rect width="320" height="180" fill="hsl(' +
      hue +
      ',40%,22%)"/>' +
      '<text x="16" y="100" font-family="Helvetica,Arial" font-size="20" ' +
      'fill="rgba(255,255,255,0.8)">YT ' +
      id +
      '</text></svg>',
  );
}
...
function embed(res, id) {
  ...
  res.end(
    '<!DOCTYPE html><title>recap</title>' +
      '<body style="margin:0;background:#000;color:#eee;font:24px sans-serif">' +
      '<p style="padding:40px">mock recap player: ' +
      id +
      '</p>',
  );
}
