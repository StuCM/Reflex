    ...
    return page.evaluate(function () {
      ...
      window.__tileArt = function () {
        ...
        for (var i = 0; i < tiles.length; i++) {
          ...
          if (state === 'fresh') out.fresh++;
          else out[state].push(title || '(no title)');
        }
        return out;
      };
  ...
  return h
    ...
    .then(function () {
      return step('posters load', function () {
        return waitFor(
          '(function(){var i=document.querySelectorAll("#rows img");' +
    ...
    .then(function () {
      return step(
        'artwork is looked up once per title, and only for what is on screen',
        function () {
          let first;
          return (
            backToLibrary()
             ...
             then walk it again: the second pass must cost nothing. */
              .then(function () {
                return press('ArrowRight', 8);
              })
              .then(function () {
                return page.waitForTimeout(1200);
              })
              .then(function () {
                first = artLookups.length;
              })
              .then(function () {
                return press('ArrowLeft', 8);
              })
              .then(function () {
              ...
              .then(function () {
                if (artLookups.length !== first) {
                  throw new Error(
                    'walking the row again cost ' + (artLookups.length - first) + ' more lookups',
                  );
                }
              })
          );
    ...
    .then(function () {
      return step(
        'a moving rail fetches only what has the focus, and fills in when it stops',
        function () {
          ...
          return (
            backToLibrary()
             ...
             exactly one new tile into the strip. */
              ...
              .then(function () {
                before = tilePosters.length;
              })
              .then(function () {
                return press('ArrowRight', SWEEP);
              })
              .then(function () {
               ...
               near that means the sweep is still fetching what it goes by. */
                ...
                if (during * 2 >= SWEEP) {
                  throw new Error(
                    during + ' poster requests while sweeping past ' + SWEEP + ' tiles',
                  );
                }
              })
              .then(function () {
        ...
        return backToLibrary()
          ...
          .then(function (readings) {
            ...
            readings.forEach(function (st, n) {
              if (st.stale.length) {
                throw new Error(
                  'press ' + (n + 1) + " left another film's poster on: " + st.stale.join(', '),
                );
              }
              if (st.focused !== 'fresh') {
                throw new Error(
          ...
          .then(function (st) {
            if (st.stale.length || st.blank.length) {
              throw new Error('after the sweep settled: ' + st.stale.concat(st.blank).join(', '));
            }
          });
      });
    })
        ...
        return searchFor(titles.oneShot.title)
          ...
          .then(function (st) {
            ...
            if (st.tile.url.indexOf('%2Fthumb%2F') < 0) {
              throw new Error(
                'the tile fell back to something other than a poster: ' + st.tile.url,
              );
            }
            if (!st.tile.painted) throw new Error('the tile drew nothing');
          })
    ...
    .then(function () {
      return step('the header carries the description and the cast, on both screens', function () {
        ...
        let head;
        return (
          searchFor(titles.manyShots.title)
            .then(function () {
              return page.waitForTimeout(1200);
            })
            .then(function () {
            ...
            .then(function () {
              return openTitle(titles.manyShots.title);
            })
            .then(function () {
              return page.evaluate(function () {
                return {
                  desc: document.getElementById('dt-summary').textContent.trim(),
      ...
      return step('a film says what it is, in chips and a kicker with no gaps', function () {
        return openTitle(titles.directPlays.title)
          .then(function () {
            ...
            return waitFor(
              'document.getElementById("dt-kicker").textContent.indexOf("·") > 0',
              'the kicker to fill in',
              15000,
            );
          })
          .then(detailFace)
    ...
    .then(function () {
      return step('every score is labelled with the source it came from', function () {
        return openTitle(titles.directPlays.title)
          .then(function () {
            return waitFor(
              'document.querySelectorAll("#dt-ratings .dt-rating").length > 0',
              'the ratings row',
              15000,
            );
          })
    ...
    .then(function () {
      return step('the All row knows its size without crawling it', function () {
        return press('ArrowDown', 5)
          .then(function () {
            return waitFor(
              '/All films\\s+\\(\\d+\\)/.test(document.querySelector("#rows").textContent)',
              'the All row count',
            );
          })
          .then(function () {
          ...
          .then(function (count) {
            ...
            if (count > FILMS * 2)
              throw new Error('claims ' + count + ', more than both servers hold');
          });
      });
    })
    ...
    .then(function () {
      return step('paging fills tiles deep into the library', function () {
        return press('ArrowRight', 40)
          ...
          .then(function () {
            return waitFor(
              '(function(){var t=document.querySelector("#rows .row.on .tile.on");' +
          ...
          .then(function (count) {
            if (count <= titles.movies.biggest) {
              throw new Error(
                'All films claims ' +
                  count +
                  ', no more than the biggest library (' +
                  titles.movies.biggest +
                  '): the libraries were not merged',
              );
            }
        ...
        function findTwin(left) {
          return page
            .evaluate(function () {
              ...
              const here = copies.filter(function (c) {
                return c._server === item._server;
              });
            ...
            .then(function (st) {
              ...
              if (left <= 0)
                throw new Error('no film with two copies on one server in the All row');
              return press('ArrowRight').then(function () {
                return findTwin(left - 1);
              });
            });
        }
        let entry;
        return findTwin(60)
          ...
          .then(function () {
            ...
            return waitFor(
              '(function(){var r=document.querySelectorAll(' +
                ...
                'return true;})()',
              'every copy of ' + entry.title + ' checked',
              20000,
            );
          })
          .then(sourceRows)
          .then(function (src) {
            ...
            src.forEach(function (s) {
              names[s.server] = (names[s.server] || 0) + 1;
            });
            const twice = Object.keys(names).filter(function (n) {
              return names[n] > 1;
            });
            if (!twice.length)
              throw new Error(
                'no server listed twice: ' +
                  src
                    .map(function (s) {
                      return s.server;
                    })
          ...
          .then(function () {
            return shot('detail-two-libraries');
          })
          .then(backToLibrary);
      });
    })
    ...
    .then(function () {
      return step('kids rows exclude everything above the cutoff', function () {
        return sidebarPick('Kids')
          .then(function () {
            return waitFor(
              '/Kids/.test(document.querySelector("#rows").textContent)',
              'a kids row',
              15000,
            );
          })
