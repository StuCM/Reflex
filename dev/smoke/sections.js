...
module.exports = function (h) {
  ...
  return h
      ...
      return step('the sidebar lists the modes and jumps to a category', function () {
        ...
        return (
          backToLibrary()
            ...
            .then(function () {
              return sidebarPick(cats[0]);
            })
            .then(function () {
              return waitFor(
                'document.getElementById("mh-row").textContent.trim() === ' +
                  JSON.stringify(cats[0]),
                'the rail to land on ' + cats[0],
              );
            })
        );
      });
    ...
    .then(function () {
      return step(
        'the sidebar lists Continue watching, Movies and TV Shows, once each',
        function () {
          return backToLibrary()
            ...
            .then(function (rows) {
              ...
              ['Continue watching', 'Movies', 'TV Shows', 'Discovery', 'Kids', 'Search'].forEach(
                function (want) {
              ...
              ['Films', '4K Films'].forEach(function (lib) {
                if (top.indexOf(lib) >= 0)
                  throw new Error('the library ' + lib + ' is still listed: ' + text);
              });
      ...
      return step('Continue watching can be cut to films or to episodes', function () {
        ...
        return (
          backToLibrary()
            ...
            .then(function () {
              return types('Continue watching');
            })
            .then(function (all) {
            ...
            .then(function () {
              return watchingPick('TV Shows');
            })
            .then(function () {
              return types('the TV Shows cut');
            })
            .then(function (only) {
              const stray = only.filter(function (t) {
                return t !== 'episode';
              });
              if (stray.length) throw new Error('the TV Shows cut kept ' + stray.join(', '));
            })
            .then(function () {
              return watchingPick('Movies');
            })
            .then(function () {
              return types('the Movies cut');
            })
            .then(function (only) {
              const stray = only.filter(function (t) {
                return t !== 'movie';
      ...
      return step("an episode tile is its show's poster, for no extra request", function () {
        ...
        return (
          backToLibrary()
            ...
            .then(function (tiles) {
              var eps = tiles.filter(function (t) {
                return t.type === 'episode';
              });
          ...
          backToLibrary()
            ...
            .then(function () {
              return waitFor(
                ...
                'the series page for the episode',
                20000,
              );
            })
            .then(function () {
            ...
            .then(function () {
              return waitFor(
                ...
                'the action row for the next episode',
                20000,
              );
            })
            .then(detailFace)
            ...
            .then(function () {
              ...
              const asks = [];
              function watch(r) {
                if (/\/library\/all\?.*guid=/.test(r.url())) asks.push(r.url());
              }
        ...
        return backToLibrary()
          ...
          .then(function () {
            return page.evaluate(function () {
              ...
              Sidebar.open(
                [{ title: 'Films', categories: many, current: true }],
                function () {},
                'library',
              );
          ...
          .then(function () {
            return page.evaluate(function () {
              ...
              return {
                focused: last.classList.contains('on'),
      ...
      return step('the Mantis palette is what the stylesheet is serving', function () {
        ...
        return page
          .evaluate(function () {
            ...
            return {
              ac: css.getPropertyValue('--ac').trim(),
        ...
        return (
          backToLibrary()
            ...
            .then(function (st) {
              if (st.hero === atA.hero) {
                throw new Error(
                  'the backdrop did not move from "' + atA.title + '" to "' + st.title + '"',
                );
              }
            })
            .then(function () {
              return press('ArrowLeft');
            }) // back to A, now cached
            .then(settle)
            .then(state)
            .then(function (st) {
              if (st.hero !== atA.hero) {
            ...
            .then(function (swept) {
              if (swept.hero === atA.hero) {
                throw new Error(
                  'after a fast sweep the backdrop is still the one from "' + atA.title + '"',
                );
              }
          ...
          backToLibrary()
            ...
            .then(function () {
              return page.evaluate(function () {
                ...
                var rows = Array.prototype.map
                  .call(document.querySelectorAll('#rows .row:not(.hidden)'), function (r) {
                    var b = r.getBoundingClientRect();
                    return {
                      height: Math.round(b.height),
                      top: Math.round(b.top),
                      shown: Math.round(Math.min(b.bottom, vp.bottom) - Math.max(b.top, vp.top)),
                    };
                  })
                  .sort(function (a, b) {
                    return a.top - b.top;
                ...
                var across = Array.prototype.filter.call(
                  row ? row.querySelectorAll('.tile:not(.hidden)') : [],
                  function (el) {
                ...
                return {
                  ...
                  art: tile
                    ? {
                        w: Math.round(tile.querySelector('.tile-inner').offsetWidth),
                        h: Math.round(tile.querySelector('.tile-inner').offsetHeight),
                      }
                    : null,
                  across: across,
                  top: t ? Math.round(t.top) : null,
                  bottom: t ? Math.round(t.bottom) : null,
                  vpTop: Math.round(vp.top),
                  vpBottom: Math.round(vp.bottom),
                  rows: rows,
                  heroOpacity: Number(
                    getComputedStyle(document.getElementById('hero-art')).opacity,
                  ),
            ...
            .then(function (st) {
              ...
              var whole = st.rows.filter(function (r) {
                return r.shown >= r.height - 1;
              });
