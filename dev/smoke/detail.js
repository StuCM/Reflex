...
module.exports = function (h) {
    ...
    h
      ...
      .then(function () {
        ...
        return step('refuses a 4K remux whose only audio would be re-encoded', function () {
          return openTitle(titles.truehdOnly.title)
            ...
            .then(function () {
              return waitFor(
                shown('4K transcode refused', titles.truehdOnly.title),
          ...
          return openTitle(titles.transcodes.title)
            .then(function () {
              return waitFor(
                ...
                "a transcode verdict on Play's caption",
                15000,
              );
            })
            .then(function () {
          ...
          return openTitle(titles.shared.title)
            ...
            .then(function (src) {
              /* The two copies must not read the same, and exactly one of them
               ...
               other. */
              const plays = src.filter(function (s) {
                return /direct play/.test(s.verdict);
              });
              const refused = src.filter(function (s) {
                return /4K, would transcode/.test(s.verdict);
              });
              if (plays.length !== 1 || refused.length !== 1) {
                throw new Error(
                  'expected one playable and one refused copy, got:\n        ' +
                    src
                      .map(function (s) {
                        return s.server + ' ' + s.version + ' — ' + s.verdict;
                      })
                      .join('\n        '),
                );
              }
      ...
      .then(function () {
        return step('a copy the guard refuses toasts and leaves the last one chosen', function () {
          ...
          return sourceRows()
            .then(function (src) {
              was = src.find(function (s) {
                return s.on;
              });
              const bad = src.find(function (s) {
                return !playable(s.verdict);
              });
            ...
            .then(sourceRows)
            .then(function (src) {
              const on = src.find(function (s) {
                return s.on;
              });
              if (!on || on.version !== was.version || on.server !== was.server) {
                throw new Error(
                  'the refused copy was selected anyway: ' +
      ...
      .then(function () {
        return step(
          'extras are cards with a length, and are guarded like anything else',
          function () {
            return (
              openTitle(titles.directPlays.title)
                .then(function () {
                  return waitFor(
                    '(function(){var e=document.querySelectorAll("#dt-extras .dt-extra");' +
                      'if (!e.length) return false;' +
                      'return !/checking/.test(e[0].textContent);})()',
                    'an extra with a verdict',
                    15000,
                  );
                })
                .then(function () {
        ...
        return step('the film page opens on Play, with a button per choice', function () {
          return openTitle(titles.directPlays.title)
            ...
            .then(function (row) {
              ...
              if (
                !/from start|resume at/.test(row[0].caption) ||
        ...
        return step('an audio track says what it costs, and the caption follows it', function () {
          return openChooser('audio')
            ...
            .then(function (labels) {
              ...
              const on = labels.filter(function (l) {
                return l.startsWith('* ');
              });
              ...
              const others = labels.filter(function (l) {
                return !l.startsWith('* ');
              });
              const quiet = others.filter(function (l) {
                return !/costs direct play/.test(l);
              });
              if (quiet.length) {
                throw new Error(
                  'a row that does not say it costs direct play: ' + labels.join(' | '),
                );
              }
            })
            .then(function () {
            ...
            .then(function () {
              return waitFor(
                '/AAC/.test(document.querySelectorAll(' +
                  '"#dt-actions .dt-act-cap")[4].textContent)',
                'the audio button to name the chosen track',
                15000,
              );
            })
            .then(actionRow)
        ...
        return step('a subtitle language chosen on the film page is what plays', function () {
          return (
            openTitle(titles.directPlays.title)
              ...
              .then(function () {
                return waitFor(
                  '/French/.test(document.querySelectorAll(' +
                    '"#dt-actions .dt-act-cap")[5].textContent)',
                  'the subtitles button to name French',
                  15000,
                );
              })
             ...
             fetches that track and draws it over the video. */
              ...
              .then(function () {
                return waitFor(
                  ...
                  'the chosen subtitle track drawn over the video',
                  15000,
                );
              })
              .then(function () {
                return shot('detail-subtitles');
              })
              .then(backToLibrary)
          );
        });
            ...
            .then(function (b) {
              if (b.body.width < 1600 || b.extras.width < 1600) {
                throw new Error(
                  'the body is ' +
                    b.body.width +
                    'px and the extras strip ' +
                    b.extras.width +
                    'px wide on a 1920 screen',
                );
              }
              ...
              if (b.body.left + b.body.width > 1920) {
                throw new Error(
                  'the body runs ' + (b.body.left + b.body.width - 1920) + 'px off the screen',
                );
              }
            })
            .then(function () {
              return shot('detail-width');
            });
        });
      })
      ...
      .then(function () {
        return step('Play is a pill sized by what it says, not a slab', function () {
          return buttonSizes().then(function (s) {
            ...
            if (s.play.width <= s.round.width) {
              throw new Error(
                'Play is ' +
                  s.play.width +
                  'px and a round button ' +
                  s.round.width +
                  'px — the primary has to lead',
              );
            }
            ...
            if (s.play.height !== s.round.height) {
              throw new Error(
                'Play is ' + s.play.height + 'px tall and a round button ' + s.round.height + 'px',
              );
            }
            if (s.round.width !== s.round.height) {
              throw new Error(
                'the round buttons are ' +
                  s.round.width +
                  '×' +
                  s.round.height +
                  ' — the pill rules have leaked into them',
              );
            }
          });
        });
        ...
        return step('the extras are a row you step down to, and step back up from', function () {
          ...
          return waitFor(
            ...
            .then(function (r) {
              ...
              if (r.bottom <= 1080) {
                throw new Error(
                  'the extras strip is fully on screen at rest — it ends at ' +
                    r.bottom +
                    ', so there is nothing to step down to',
                );
              }
              if (1080 - r.top > 160) {
                throw new Error(
                  1080 -
                    r.top +
                    'px of the extras strip shows before it is ' +
            ...
            .then(function () {
              return waitFor(
                'document.getElementById("dt-extras").getBoundingClientRect().bottom' + ' <= 1080',
                'the extras strip to come up whole',
                5000,
              );
            })
            .then(extrasBox)
            .then(function (r) {
              if (r.top >= peek.top) {
                throw new Error(
                  'stepping down moved the extras strip to ' +
                    r.top +
                    ', which is no higher than the ' +
                    peek.top +
                    ' it was at',
                );
              }
            ...
            .then(function () {
              return shot('detail-extras-down');
            })
            .then(function () {
            ...
            .then(function () {
              return waitFor(
                '!!document.querySelector("#dt-actions .dt-act.on") &&' +
                  ' !document.querySelector("#dt-extras .dt-extra.on")',
            ...
            .then(function () {
              return waitFor(
                'document.getElementById("dt-extras").getBoundingClientRect().bottom' + ' > 1080',
                'the extras strip to drop back to its peek',
                5000,
              );
            })
            .then(backToLibrary);
      ...
      .then(function () {
        return step(
          'a part-watched film offers play from the start, and it starts at 0',
          function () {
            let mark;
            return (
              backToLibrary()
                ...
                .then(function () {
                  return waitFor(
                    ...
                    'the film page for a part-watched film',
                    20000,
                  );
                })
                .then(actionRow)
                .then(function (row) {
                  ...
                  if (acts.indexOf('start') < 0) {
                    throw new Error(
                      'no play-from-start button on a part-watched film: ' + acts.join(', '),
                    );
                  }
                ...
                .then(function () {
                  return waitFor(
                    '(function(){var c=document.querySelector("#dt-actions .dt-act-cap");' +
                      'return !document.getElementById("detail").classList.contains("hidden")' +
                      ' && c && !/checking/.test(c.textContent);})()',
                    'the film page back after stopping',
                    20000,
                  );
                })
                .then(function () {
  ...
  function buttonSizes() {
    return page.evaluate(function () {
      ...
      return {
        play: size('#dt-actions .dt-act.primary .dt-act-btn'),
  ...
  function startedAt(mark) {
    const said = trace
      .slice(mark)
      .filter(function (l) {
        return /starting at \d+s/.test(l);
      })
      .map(function (l) {
        return l.replace(/^.*starting at /, '');
      });
    if (said.length !== 1) {
      throw new Error(
        'the app reported ' + said.length + ' start positions: ' + (said.join(' | ') || 'none'),
      );
    }
    return said[0];
  }
  ...
  function focusResumableFilm() {
    return page
      ...
      .then(function (row) {
        ...
        return press(row.to > row.focus ? 'ArrowRight' : 'ArrowLeft', Math.abs(row.to - row.focus));
      });
  }
};
