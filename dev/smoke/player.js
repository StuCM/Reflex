...
module.exports = function (h) {
  ...
  function focusControl(id) {
    return (
      controlRow()
        ...
        .then(function (row) {
          return row.open ? press('Backspace').then(controlRow) : row;
        })
        .then(function (row) {
          return row.foc >= 0 ? row : press('ArrowDown').then(controlRow);
        })
        .then(function (row) {
  ...
  /* The OSD clock reads where a seek is AIMED, so an assertion on it does not
     ...
     focus mode breaks. */
  const CLOCK =
    '(function(){var p=document.getElementById("osd-time").textContent' +
    '.trim().split(/\\s+/)[0].split(":").map(Number);' +
    'return p.length===3?p[0]*3600+p[1]*60+p[2]:p[0]*60+p[1];})()';
  function clock(test, what) {
    return waitFor(CLOCK + test, what);
  }
    ...
    h
        ...
        return step('up takes the trackbar, down the row, and only OK opens a panel', function () {
          return (
            press('ArrowUp')
              .then(barFocused)
              .then(function (on) {
                if (!on) throw new Error('up did not focus the trackbar');
              })
              .then(controlRow)
              .then(function (row) {
             ...
             between the two modes, in one press. */
              ...
              .then(function () {
                return waitFor(
                  '/SEEKING/.test(document.getElementById("osd-time").textContent)',
             ...
             open whatever the row was sitting on. */
              ...
              .then(function (shut) {
                if (!shut) throw new Error('up opened a panel');
              })
              .then(barFocused)
              .then(function (on) {
        ...
        return step('every documented key still means what it says unfocused', function () {
          ...
          return (
            press('Enter')
              .then(function () {
                return waitFor(
                  '/PAUSED/.test(document.getElementById("osd-time").textContent)',
                  'OK to pause',
                );
              })
              .then(function () {
              ...
              .then(function () {
                return clock(' === 0', 'the clock back at the start');
              })
              .then(function () {
                return press('5');
              })
              .then(function () {
                return clock(' > 10 && ' + CLOCK + ' < 20', 'a digit to jump');
              })
              .then(function () {
                return press('ArrowLeft');
              })
              .then(function () {
                return clock(' === 0', 'left to nudge back');
              })
              .then(function () {
                return press('ArrowRight');
              })
              .then(function () {
                return clock(' > 20', 'right to nudge on');
              })
              .then(function () {
                return press('PageDown');
              })
              .then(function () {
                return clock(' === 0', 'CH− to step back a chapter');
              })
              .then(function () {
                return press('PageUp');
              })
              .then(function () {
                return clock(' > 20', 'CH+ to step on a chapter');
              })
              /* None of that may have moved the focus: the arrows only belong to the
             bar and the row once one of them has been asked for. */
              .then(barFocused)
              .then(function (on) {
                if (on) throw new Error('seeking focused the trackbar');
              })
              .then(controlRow)
              .then(function (row) {
              ...
              .then(function () {
                return waitFor('!document.getElementById("video").paused', 'OK to start it again');
              })
          );
        });
      })
          ...
          return page
            ...
            .then(function (w) {
              ...
              if (w.right.join() !== '76,76,76,76') {
                throw new Error(
                  'the choice buttons are ' + w.right.join(', ') + ', not four of 76',
                );
              }
            })
            .then(function () {
              return openMenu('quality');
            })
            .then(function () {
              return page.evaluate(function () {
        ...
        return step('four captioned buttons, each opening its own panel', function () {
          return press('ArrowDown') // into the row
            ...
            .then(function (row) {
              ...
              const blank = row.caps.slice(row.caps.length - 4).filter(function (c) {
                return c === '';
              });
            ...
            .then(function (labels) {
              ...
              if (labels.length !== 3 || !/English/.test(labels[0]) || !/French/.test(labels[2])) {
            ...
            .then(function (labels) {
              ...
              const image = labels.filter(function (l) {
                return /image/.test(l);
              });
            ...
            .then(menuLabels)
            .then(function (labels) {
              if (!/Original/.test(labels[0]))
                throw new Error('quality rows: ' + labels.join(' | '));
          ...
          return (
            openChapters()
              ...
              .then(function (cards) {
                ...
                const sizes = cards.filter(function (c) {
                  return c.h !== cards[0].h || c.w !== cards[0].w;
                });
               ...
               broken one behind, and a background is how that is guaranteed. */
                ...
                if (!cards[0].time || !cards[0].title)
                  throw new Error('a card is missing its text');
                if (
                  cards.filter(function (c) {
                    return c.on;
                  }).length !== 1
      ...
      .then(function () {
        ...
        return step('subtitles are fetched as text and drawn over the video', function () {
          return (
            openMenu('subs')
              .then(function () {
                return menuChoose(/French/);
              })
              .then(function () {
                return waitFor(
                  '/français/.test(document.getElementById("subtitle").textContent)',
                  'the French subtitle track to be drawn over the video',
                  15000,
                );
              })
             ...
             the one thing this screen must not do. */
              ...
              .then(function (at) {
                ...
                if (at[0] > at[1]) {
                  throw new Error(
                    'the subtitle runs into the controls: bottom ' +
                      at[0] +
                      ' against an OSD starting at ' +
                      at[1],
                  );
                }
              })
                ...
                openMenu('audio')
                  ...
                  .then(function (labels) {
                    ...
                    if (/restarts/.test(labels.join(' '))) {
                      throw new Error(
                        'offered a restart for a track the panel can select: ' + labels.join(' | '),
                      );
                    }
                  })
                  .then(function () {
                    return menuChoose(/French · AAC/);
                  })
                  .then(function () {
                    return waitFor(
                      '/French/.test(document.querySelector(' +
                        '"#osd-ctl-audio .osd-cap").textContent)',
                      'the audio button to name the new track',
                      10000,
                    );
                  })
                  .then(function () {
                  ...
                  .then(function (enabled) {
                    if (!enabled[2] || enabled[0] || enabled[1]) {
                      throw new Error(
                        'the panel track was not selected: ' + JSON.stringify(enabled),
                      );
                    }
                    const after = trace.slice(before);
                    if (
                      after.filter(function (l) {
                        return /decision:/.test(l);
                      }).length
                    ) {
          ...
          return (
            openMenu('audio')
              ...
              .then(function (labels) {
                ...
                if (!/restarts/.test(labels.join(' '))) {
                  throw new Error(
                    'no restart warning without a panel track list: ' + labels.join(' | '),
                  );
                }
              })
             ...
             here is a failure the harness cannot avoid. */
              ...
              .then(function () {
               ...
               unrelated parts. */
                const verdict = after.find(function (l) {
                  return l.indexOf('decision:') >= 0;
                });
               ...
               and this is what it looked like in the trace. */
                ...
                if (
                  !after.filter(function (l) {
                    return /server converting/.test(l);
                  }).length
                ) {
                  throw new Error(
                    'played the original file again rather than the muxed stream: ' +
                      after.slice(-3).join(' | '),
                  );
                }
              })
        ...
        return step('skip intro appears inside the marker and takes you past it', function () {
          ...
          return (
            press('ArrowLeft')
              .then(function () {
                return waitFor(
                  '!document.getElementById("osd-skip").classList.contains("hidden")',
                  'the skip-intro offer',
                  20000,
                );
              })
              .then(function () {
                return page.textContent('#osd-skip');
              })
              .then(function (text) {
                if (!/Skip intro/.test(text)) throw new Error('the offer says: ' + text);
              })
              .then(function () {
                return shot('skip-intro');
              })
             ...
             button is not pause, and the moment you are reaching for it. */
              ...
              .then(function () {
                return waitFor(
                  'document.getElementById("video").currentTime >= 11.5',
                  'playback to land past the end of the intro',
                  10000,
                );
              })
              .then(function () {
        ...
        return step('the trackbar shows chapters, and a digit jumps', function () {
          return (
            page
              .evaluate(function () {
                return [
                  document.querySelectorAll('#osd-ticks .osd-tick').length,
      ...
      .then(function () {
        ...
        return step('the OSD follows playback, and Back stops it', function () {
          var at;
          return waitFor(
            'document.getElementById("osd-time").textContent.trim().indexOf("0:00") !== 0',
            'the OSD clock to move off zero',
            15000,
          )
            .then(function () {
              return page.evaluate(function () {
      ...
      .then(function () {
        ...
        return step('an episode ending offers the next one, and waits', function () {
          return (
            backToLibrary()
              .then(function () {
                return openShowPage(titles.run.title);
              })
              .then(function () {
        ...
        return step('OK on the offer plays the next one, the last one closed first', function () {
          ...
          return page.keyboard
            ...
            .then(function () {
              ...
              const stopped = after.findIndex(function (u) {
                return /state=stopped/.test(u);
              });
              const playing = after.findIndex(function (u) {
          ...
          return playToEnd()
            ...
            .then(function () {
              return waitFor(
                ...
                'the refusal, back on the series page',
                25000,
              );
            })
            .then(function () {
      ...
      .then(function () {
        return step('the sidebar carries the autoplay setting and cycles it', function () {
          return backToLibrary()
            ...
            .then(function () {
              return sidebarPick('Autoplay next: off');
            })
            .then(function () {
              return waitFor(
                '/Autoplay next: 5s/.test(document.getElementById("toast").textContent)',
            ...
            .then(function () {
              ...
              return page.evaluate(function () {
                return localStorage.getItem('reflex.autoplay');
              });
            })
        ...
        return step('the countdown runs, and BACK before zero cancels it', function () {
          return (
            backToLibrary()
              ...
              .then(function () {
                return waitFor(
                  '/Playing in/.test(document.getElementById("un-hint").textContent)',
                  'the countdown',
                  25000,
                );
              })
             ...
             behind a screen nobody is looking at any more. */
              .then(function () {
                return page.waitForTimeout(6000);
              })
              .then(function () {
                return page.evaluate(function () {
              ...
              .then(function () {
                return waitFor(
                  '(function(){var v=document.getElementById("video");' +
                    'return document.getElementById("upnext").classList.contains("hidden") &&' +
                    ' !v.classList.contains("hidden") && v.currentTime > 0.2 && !v.error;})()',
                  'the next episode to start itself',
                  25000,
                );
              })
              .then(function () {
      ...
      .then(function () {
        ...
        return step('a new series is offered but never counted down', function () {
          return backToLibrary()
            ...
            .then(function () {
              return playEpisode(1, titles.run.lastOfFirst);
            })
            .then(playToEnd)
            .then(waitForOffer)
            .then(function (st) {
      ...
      .then(function () {
        ...
        return step('the last episode of the last series offers nothing', function () {
          return backToLibrary()
            ...
            .then(function () {
              return playEpisode(titles.run.seasons, titles.run.lastOfLast);
            })
            .then(playToEnd)
            .then(function () {
              return waitFor(
                'document.getElementById("video").classList.contains("hidden") &&' +
                  ' document.getElementById("upnext").classList.contains("hidden") &&' +
                  ' !document.getElementById("show").classList.contains("hidden")',
                'playback to end with nothing offered',
                25000,
              );
            });
        });
            ...
            .then(function () {
              return waitFor(
                'document.getElementById("video").classList.contains("hidden") &&' +
                  ' document.getElementById("upnext").classList.contains("hidden") &&' +
                  ' !document.getElementById("detail").classList.contains("hidden")',
                'the film to end and the page to come back',
                25000,
              );
            });
        });
