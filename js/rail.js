...
var Rail = (function () {
  ...
  function drawRow(rowEl, rows, r, rowIdx, onScreen) {
    ...
    const firstVisible = UI.clamp(row.focus - 3, 0, Math.max(0, row.total - TILES_VISIBLE));
    const start = UI.clamp(firstVisible - 2, 0, Math.max(0, row.total - TILE_POOL));
    ...
    for (let i = 0; i < TILE_POOL; i++) {
      ...
      tile._idx = idx;
      const item = Rows.itemAt(row, idx);
      tile._filled = !!item;
      if (!item) {
      ...
      const url = Plex.posterUrl(item, TILE_W, TILE_H);
