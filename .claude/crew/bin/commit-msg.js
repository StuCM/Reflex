...
if (!m) {
  ...
  if (/^[A-Z][a-z]/.test(summary)) {
    errors.push(
      'subject starts lowercase: "' + summary.charAt(0).toLowerCase() + summary.slice(1) + '"',
    );
  }
  if (
    /^(added|adds|fixed|fixes|updated|updates|changed|changes|removed|removes|refactored)\b/i.test(
      summary,
    )
  ) {
