...
const ep = { type: 'episode', title: 'Spirit Receivers', grandparentTitle: 'Archive 81',
             parentIndex: 1, index: 4, duration: 2760000 };

assert.strictEqual(Media.railTitle(ep), 'Archive 81', 'an episode is named by its show');
assert.strictEqual(Media.railSub(ep), 'S1 E4  ·  Spirit Receivers',
...
assert.strictEqual(Media.railSub({ type: 'episode', title: 'Pilot' }), 'Pilot',
                   'no numbers at all falls back to the episode title');
assert.strictEqual(Media.railTitle({ type: 'episode', title: 'Pilot' }), 'Pilot',
...
assert.strictEqual(Media.railSub({ type: 'movie', title: 'X' }), '',
...
assert.strictEqual(Media.railSub({ type: 'show', title: 'X', year: 1999 }), '1999',
